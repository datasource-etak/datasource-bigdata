...
public class DatastorePostgresqlConnector implements DatastoreConnector {
    ...
    public void put(MasterData masterData, boolean withCreate) throws Exception {
        try {
            ...
            String infopart = (withCreate) ? "" : "out";
            LOGGER.log(Level.INFO, "Inserting to master data with" + infopart + " table creation.");
            ...
            for (DimensionTable table: masterData.getTables()) {
                ...
                if (withCreate) {
                    String q = "CREATE TABLE IF NOT EXISTS " + table.getName() + " (";
                    for (KeyValue element : columns) {
                        q += element.getKey() + " " + element.getValue();
                        if (element.getKey().equals(primaryKey)) q += " PRIMARY KEY";
                        q += ",";
                    }
                    q = q.substring(0, q.length() - 1) + ");";
                    LOGGER.log(Level.INFO, q);
