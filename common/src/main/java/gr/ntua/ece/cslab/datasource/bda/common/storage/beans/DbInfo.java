...
public class DbInfo implements Serializable {
    ...
    private final static String SEARCH_FOR_DBINFO_EXISTS_QUERY =
            "SELECT EXISTS (SELECT 1 FROM db_info WHERE slug = ? OR name = ?) AS ALREADY_EXISTS;";
    ...
    public static boolean searchForUserspace(String slug, String name) throws SQLException, SystemConnectorException {

        PostgresqlConnector connector = (PostgresqlConnector ) SystemConnector.getInstance().getBDAconnector();
        Connection connection = connector.getConnection();

        try {
            PreparedStatement statement = connection.prepareStatement(SEARCH_FOR_DBINFO_EXISTS_QUERY);
            statement.setString(1, slug);
            ...
            if (resultSet.next()) {
                ...
                return resultSet.getBoolean("ALREADY_EXISTS");
